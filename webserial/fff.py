...
class FanFicFare:
    ...
    def download_serial(
        self, ebook: FilePath, url: HttpUrl, update: bool = True, force: bool = False
    ) -> str:
        command = [
            "fanficfare",
        ]
        ...
        if more_chapters.search(output):
            raise LocalAheadOfRemoteError(
                "Your local file has been updated more recently than the story."
            )
        if error:
            raise WebserialError(error)
